// $Id: IPProtocol.java,v 1.1 2005/07/22 14:13:11 mpelze2s Exp $
...
public class IPProtocol implements IPProtocols
{
   ...
   */
  public static int extractProtocol(int lLen, byte [] packetBytes, int ipVersion) {
    if (ipVersion == EthernetProtocols.IP) {
  		// IPv4
  		//* DEBUG */ System.out.println("Pos: " + IPFields.IP_CODE_POS);
    	return packetBytes[lLen + IPFields.IP_CODE_POS];
  	}
    else {
    	// IPv6
		//* DEBUG */ System.out.println("Pos: " + (IPFields.IPv6_NEXT_HEADER_POS / 8));
		return packetBytes[lLen + (IPFields.IPv6_NEXT_HEADER_POS / 8)]; // IPv6 positions are given in bits -> / 8
    }
  }
